...
const GEN_API_KEY = 'AIzaSyCmhMTdIHTayA0jRpVWmMb2Mt2OjOzGceA';//process.env.GEN_API_KEY;
if (!GEN_API_KEY) {
...
exports.generateQuestion = async (req, res) => {
    try {
        ...
        const cooldownTime = 5000; // 5 giây
        if (lastRequestTime && now - lastRequestTime < cooldownTime) {
            const remainingTime = Math.ceil((cooldownTime - (now - lastRequestTime)) / 1000);
        ...
        const googleGenerativeAI = new GoogleGenerativeAI(GEN_API_KEY);
        const model = googleGenerativeAI.getGenerativeModel({model: 'gemini-2.0-flash'});

        // Tạo prompt dùng 1 template duy nhất:
        ...
        // Gọi API Google Generative AI với prompt đã tạo
        const result = await model.generateContent(prompt);
        const rawMarkdown = result?.response?.candidates?.[0]?.content?.parts?.map(part => part.text).json('') || 'No response from AI.';
        // Chuyển đổi markdown sang HTML
        const htmlContent = marked(rawMarkdown);

        // Trả về câu hỏi đã tạo
        res.json({questions: htmlContent, jdText});
    } catch (error) {
